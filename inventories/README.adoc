...
====
Explanations::
* Do not use playbook and play variables, as well as include_vars, prefer inventory variables.
* Avoid using scoped variables unless you need for runtime reasons (loop and similar). 

Rationale::
There are https://docs.ansible.com/ansible/latest/user_guide/playbooks_variables.html#understanding-variable-precedence[22 levels of variable precedence], this is almost impossible to keep in mind for a "normal" human and can lead to all kind of weird behaviours if not under control.
It also needs to be noted that many variable types tend to blur the separation between code and data and are to be avoided.
By reducing the number of variable types, you come to a more simple and overseeable list of variables, with explanations why they have their specific precedence, so that they become easier to remember and use wisely:
+
...
. and extra_vars overwrite everything else (even runtime vars, which can be quite surprising)

The following picture shows how this can be summarized in a simplified way to keep in mind, highlighting which variables are meant to overwrite each other:
...
== Prefer inventory variables for desired state variables over extra vars
[%collapsible]
====
Explanations::
Don't use extra vars to define your desired state.
Make sure your inventory contains the complete state of your environment as you want to reach.
