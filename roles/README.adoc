...
+
Use instead a variable (a list variable in the first case), or at least make thegroup's name a parameter of your role.
...
You should better rely on the inventory parsing process to provide your code with the variables it needs, not enforcing a specific structure of the inventory.
...
The other approaches are good to make sure each action applies only once, at the price of many skips.
...
+
Overall, it is best if you can avoid this kind of constructs as they are clumsy, but it depends on the use case if it's possible.
====
