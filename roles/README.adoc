...
A role should have a single entry point. Requiring users to use `tasks_from: file.yml` is discouraged as this could cause important pre and post task files executed in `tasks/main.yml` to be skipped.
The `state` or `action` keyword should be passed to a role when a role has such a capability and be required when calling the role. The `tasks/main.yml` should load the appropriate task file based on the action or state requested by the user, rather than the consumer having to have intimate knowledge of the file structure layout within the role.
All facts added by a role should fall under a single fact key named as the role.
All roles should use argspec validation, if multiple states or actions are supported argspec validation should be done with a task.
All roles should use argspec validation at the egress to confirm facts are correctly set.
The tasks folder or one of its subdirectories should not contain more than 10 files.
All intermediate facts set during role execution should be prefaced with the role name.
All roles should be packaged within an ansible collection
Configuration management roles should be atomic, idempotent, and rollback when failures are encountered
When many roles within a collection are similar, consider using a dispatcher role as the single entry point to invoke a specific role within the collection. An example of such may be a collection that does system health checks and contains many roles, each representing an individual health check.
====
...
* All roles should have a complete README and meta/main.yml
* All entries in a defaults/main.yml should be documented with comments
* The ingress and egress argspec should be programmatically included in the README
====
...
A README file should be included in the root directory of the role that introduces the user to the purpose of the role and any important information on how to use it, such as credentials that are required.
The role’s README should be programmatically maintained and kept up to date.
