...
NOTE: this section has been rewritten, using https://github.com/oasis-roles/meta_standards[OASIS metastandards repository] and https://docs.google.com/document/d/1jZUsman8IDSvDt-s6loJ3Y4uZsyse3CB-xzM2DCYaHQ/edit[Ansible Good Practice Recommendations] as a starting place. If you have anything to add or review, please comment.
...
However, when the underlying implementations become too divergent, for example implementing an email server with postfix or sendmail, then separate roles are encouraged.
...
[%collapsible]
====
Explanations:: New roles should be initiated in line, with the skeleton directory, which has standard boilerplate code for a Galaxy-compatible Ansible role and some enforcement around these standards
...
Rationale:: There are some https://github.com/ansible/ansible/issues/67512[restrictions] for Ansible Galaxy and Automation Hub.  The versioning must be in strict X.Y.Z[ab][W] format, where X, Y, and Z are integers.
