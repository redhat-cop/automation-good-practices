...
Try to design the interface focused on the functionality, not on the software implementation behind it. Roles should accomplish a guaranteed outcome; the scope of a role should be its outcome
...
Do not use roles as programming interfaces, in the case a common role is necessary to contain common code available to other roles within the same collection, it should not contain user-facing plugins.
Keep roles loosely coupled with other external roles and variables.
====
