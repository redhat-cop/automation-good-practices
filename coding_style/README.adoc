...
* A single jinja template should not exceed 250 lines in length
* The aggregate number of jinja includes should not exceed 3
* Jinja templates should not be used to create structured data but instead text and semi-structured data
* Filter plugins are preferred over the use of data manipulation or transformation in Jinja templates when the necessary data transformation cannot be accomplished with existing and available filter plugins or the transformation of data is unique to the use case.
+
+
[%collapsible]
====
Rationale:: Long and complex Jinja templates can be difficult to maintain and debug. When working with structured data and/or data transformations it is preferable to use a programming language (such as Python) that has better support and tooling to do this kind of work. Additionally, tasks will be much more legible if data is managed and manipulated via plugins than with in-line Jinja.
====
