...
[%collapsible]
====
Rationale:: Use of whitespace and multi-line indentation makes Jinja expressions easier to read.
...
[%collapsible]
====
Rationale:: Best way to wrap initial lines that cannot be otherwise wrapped
...
[%collapsible]
====
Rationale:: Set intermediate values in `vars` to simplify Jinja expressions.
Especially when the value is used multiple times in a task/block.

But what if my variable name is very long, and/or I use it in several places?
Use a `vars` in the task to assign a locally scoped variable with a short name, or pre-digest some of the work.
This is very useful in a `block` where you have a value used in several places:
...
Remember that Ansible uses lazy evaluation for `vars`, so you can define `vars` in a `block` where the ultimate value can depend on values set by subsequent tasks in the `block`.
====
...
[%collapsible]
====
Rationale:: Use backslash for difficult line wrapping cases.
